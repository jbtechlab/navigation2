...
PlannerServer::computePlanThroughPoses()
{
  ...
  if(goal->planner_ids.size() != goal->goals.size() && goal->planner_ids.size() != 0 && goal->planner_ids.size() != 1){
    RCLCPP_WARN(
      get_logger(),
      "Compute path through poses requested with a planner_ids array length mismatch. It should be either 0 - use default, 1 - use one for all, or length should be equal to the number of goals.");
    action_server_poses_->terminate_current();
  }
